...
public class MainActivity extends Activity {
    ...
    private void findInterest(DataSnapshot dataSnapshot) {
        SE = dataSnapshot.getValue(User.class).isSE();
        oop = dataSnapshot.getValue(User.class).isOop();
        db = dataSnapshot.getValue(User.class).isDatabase();
        ui = dataSnapshot.getValue(User.class).isDesign();
    }
        ...
        potentialMatch.addChildEventListener(new ChildEventListener() {
            ...
            public void onChildAdded(DataSnapshot dataSnapshot, String s) {
                if (dataSnapshot.exists() && !dataSnapshot.child("connections").child("dislikeme").hasChild(currentUID) && !dataSnapshot.child("connections").child("likeme").hasChild(currentUID) && !dataSnapshot.getKey().equals(currentUID)) {
                    ...
                    boolean tempDatabase = curUser.isDatabase();
                    boolean tempOop = curUser.isOop();
                    boolean tempDesign = curUser.isDesign();
                    boolean tempSE = curUser.isSE();
                    ...
                    if (tempOop == oop || tempDesign == ui || tempDatabase == db || tempSE == SE) {
                        ...
                        Cards item = new Cards(dataSnapshot.getKey(), username, dob, age, profileImageUrl, bio, interest.toString(), distance);
                        rowItems.add(item);
                        arrayAdapter.notifyDataSetChanged();
                    }
                }
